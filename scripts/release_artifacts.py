...
SCHEMA_VERSION = "GHC-PUBLIC-RELEASE-1.0"
ARCHIVE_STEM = "grounded-hypercalculi"
DISTRIBUTION_NAME = "grounded-hypercalculi"
IMPORT_PACKAGE = "grounded_hypercalculi"
...
def _repository_url(repo: Path) -> str:
    try:
        ...
        return f"https://github.com/TimeLordRaps/{ARCHIVE_STEM}"
...
def build_source(repo: Path, ref: str, release: str, output_dir: Path) -> tuple[Path, Path]:
    ...
    with tempfile.TemporaryDirectory(prefix="ghc-source-archive-") as temporary:
    ...
    manifest = {
        ...
        "release": release,
        "scope": (
            "Grounded Hypercalculi specifications, core implementations, "
            "formal bridges, examples, tests, and public project process"
        ),
        "source": {
...
def _build_wheel_once(source_archive: Path, prefix: str, destination: Path, epoch: str) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="ghc-wheel-") as temporary:
...
def _build_sdist_once(
    ...
    destination.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="ghc-sdist-") as temporary:
        temporary_path = Path(temporary)
        with zipfile.ZipFile(source_archive) as bundle:
...
def build_all(
    ...
    with tempfile.TemporaryDirectory(prefix="ghc-wheel-builds-") as temporary:
        temporary_path = Path(temporary)
    ...
    with tempfile.TemporaryDirectory(prefix="ghc-sdist-builds-") as temporary:
        temporary_path = Path(temporary)
